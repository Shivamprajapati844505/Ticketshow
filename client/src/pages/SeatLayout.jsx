...
const SeatLayout = () => {
  ...
  try {
    ...
    const { data } = await axios.get(
      `/bookings/seats/${selectedTime.showId}`,
      {
        headers: {
    ...
    toast.error("Cannot load booked seats");
  }
};
  ...
  const bookTickets = async ()=>{
    try {
         ...
         const {data}  = await axios.post('/bookings/create',{showId:selectedTime.showId, selectedSeats}, {headers:{
  ...
  return show ? (
    <div className="flex flex-col  md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50">
      <div className="w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-hm md:sticky md:top-30">
